...
Usage:
    ...
    python -m scripts.review_aliases --unreviewed-only
"""
...
def fetch_unreviewed_aliases(since: datetime | None = None) -> list[dict]:
    """Return aliases not yet present in examples.json."""
    db = SessionLocal()
    try:
        rows = (
            db.query(
...
def update_alias_in_db(id_alias: int, new_name: str, new_category: str,
                        new_brand: str | None) -> None:
    """Update normalized name, category and brand for a given alias."""
    db = SessionLocal()
    try:
        alias = db.query(ProductAlias).filter_by(id_alias=id_alias).first()
        if not alias:
            print(f"{RED}Alias {id_alias} not found in DB.{RESET}")
            return
...
def add_to_examples(entry: dict, data: dict) -> None:
    ...
    existing = next(
        (e for e in examples if e["original_name"].upper() == entry["original_name"].upper()),
        None
    )
    if existing:
...
# ─────────────────────────────────────────────
# Available categories (must match prompt)
# ─────────────────────────────────────────────

CATEGORIES = [
    "Lácteos", "Carnes", "Pescados", "Frutas", "Verduras",
    "Panadería y pastelería", "Bebidas", "Droguería", "Higiene",
    "Congelados", "Snacks", "Huevos", "Cereales y pasta",
...
def pick_category(current: str) -> str:
    ...
    while True:
        raw = input(f"  Enter number (or press Enter to keep '{current}'): ").strip()
        if not raw:
            return current
...
def review(since: datetime | None = None) -> None:
    aliases   = fetch_unreviewed_aliases(since)
    examples  = load_examples()

    # Filter out aliases already in examples.json
    pending = [
        a for a in aliases
    ...
    if not pending:
        print(f"{GREEN}Nothing to review — all aliases are already in examples.json.{RESET}")
        return
    ...
    print(f"  Commands: {GREEN}y{RESET}=approve  {YELLOW}e{RESET}=edit  "
    ...
    for i, alias in enumerate(pending, 1):
        ...
        while True:
            ...
            if cmd == "y":
                # Approve as-is → add to examples.json
                add_to_examples({
                ...
                # Edit → fix in DB and examples.json
                ...
                update_alias_in_db(alias["id_alias"], new_name, new_category, new_brand)
                add_to_examples({
                    "original_name": alias["original_name"],
                    "name":          new_name,
                ...
                # Discard — don't add to examples.json, leave DB as-is
...
def _print_summary(approved: int, corrected: int, discarded: int, skipped: int) -> None:
    print(f"{BOLD}═══════════════════════════════════════════{RESET}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Only review aliases from tickets processed after this date (YYYY-MM-DD)",
        default=None,
    )
